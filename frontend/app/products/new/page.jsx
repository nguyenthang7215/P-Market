...
export default function CreateProductPage() {
  const { isConnected } = useWallet(); // <-- Lấy từ context
        ...
        <Card className="max-w-2xl mx-auto shadow-lg">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Select id="category" className="mt-1" value={category} onChange={(e) => setCategory(e.target.value)}>
                    <option value="books">Sách vở</option>
                    <option value="electronics">Đồ điện tử</option>
                    <option value="housing">Phòng trọ</option>
                    <option value="fashion">Thời trang</option>
                    <option value="free">Cho tặng (Nhận Green Credit)</option>
                  </Select>
                </div>
                <div>
                  <label htmlFor="condition" className="block text-sm font-medium">Tình trạng</label>
                  <Select id="condition" className="mt-1">
                    <option>Mới (Chưa sử dụng)</option>
                    <option>Đã qua sử dụng - Như mới</option>
                    <option>Đã qua sử dụng - Tốt</option>
                    <option>Đã qua sử dụng - Có lỗi nhỏ</option>
                  </Select>
                </div>
              </div>
              {category === 'free' && (
                <div className="p-3 bg-green-50 border border-green-200 rounded-md flex items-center gap-3 text-green-800">
                  <Leaf size={20} /><p className="text-sm font-medium">Hành động đẹp! Bạn sẽ được cộng <strong>5 Green Credit</strong> khi đăng bài này.</p>
                </div>
              )}
              <div>
                <label htmlFor="description" className="block text-sm font-medium">Mô tả chi tiết</label>
                <Textarea id="description" placeholder="Mô tả chi tiết tình trạng, ngoại hình, lỗi (nếu có)..." className="mt-1" />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="price" className="block text-sm font-medium">Giá (VNĐ)</label>
                  <Input id="price" type="number" placeholder="0" className="mt-1" value={price} onChange={(e) => setPrice(e.target.value)} readOnly={category === 'free'} required />
                </div>
                <div>
                  <label htmlFor="quantity" className="block text-sm font-medium">Số lượng</label>
                  <Input id="quantity" type="number" defaultValue="1" className="mt-1" required />
                </div>
              </div>
              <div>
                <label htmlFor="image" className="block text-sm font-medium">Đăng hình ảnh</label>
          ...
          <CardFooter>
            <Button type="submit" className="w-full" size="lg" disabled={!isConnected}>
              {isConnected ? 'Hoàn tất & Đăng bài' : 'Đăng bài (Cần liên kết ví)'}
            </Button>
          </CardFooter>
        </Card>
      </form>
